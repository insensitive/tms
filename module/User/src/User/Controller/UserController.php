...
use User\FormFilter\AddFormFilter;
...
class UserController extends AbstractController
{
    ...
    public function addAction ()
    {
        ...
        $view->setTemplate('user/user/add_edit');
        return $view;
    }
    ...
    public function editAction ()
    {
        ...
        $role = $form->get('afc_group_id');
        $role->setValue($groupId);
    ...
    public function userGridListAction ()
    {
        $request = $this->getRequest();
        $options = array(
            'column' => array(
        ...
        foreach ($aaData as $key => &$row) {
            if (strtolower($row[0]) == "guest" || strtolower($row[0]) == "administrator") {
                unset($aaData[$key]);
            ...
            $row[5] = $bootstrapLinks->gridEditDelete($this->url()
                ...
                'delete' => array(
                    'attributes' => array(
                        'data-target-id' => $allData['user_id'],
                        'data-target' => '#delete-template',
                        'data-toggle' => 'modal',
                        'data-target-name' => $allData['firstname']
                    ),
                    'class' => array(
                        'delete'
                    )
